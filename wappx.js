...
jid:user:5534XXXXXXXX
Status:200 Exist
window.Store.WapQuery.queryExist("5534XXXXXXXX@c.us");
...
window.WAPPX.sendImageFromDatabasePicBot = function (picId, chatId, caption) {
    ...
    var chatSend = WAPPX.getChat(chatId);
...
window.WAPPX.getChatById = function (id, done) {
    let found = window.WAPPX.getChat(id);
    if (found) {
...
window.WAPPX.loadEarlierMessages = function (id, done) {
    const found = window.WAPPX.getChat(id);
    if (done !== undefined) {
...
window.WAPPX.sendMessage = function (id, message, done) {
    var chat = window.WAPPX.getChat(id);
    if (chat !== undefined) {
        if (done !== undefined) {
            chat.sendMessage(message).then(function () {
                function sleep(ms) {
...
window.WAPPX.sendContact = function(to, contact) {
    ...
    contact = contact.map((c) => {
        return window.WAPPX.getChat(c).__x_contact;
    });
